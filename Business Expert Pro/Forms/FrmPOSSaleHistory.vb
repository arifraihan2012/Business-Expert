...
Public Class FrmPOSSaleHistory
    ...
    Sub RefreshSaleDetails()
        ...
        Try
            ...
            Dim connectionString As String = FrmHome.ConnectionString
            Dim sql As String = "SELECT [Sale].[Sale ID],[Sale].[Sale Invoice NO],[Sale].[Purchase ID],[Products].[Product Name],[Sale].[Sale Quantity],[Sale].[Sale Unit Price],[Sale].[Sale Amount] from [Sale] join [Products] on [sale].[product ID]=[products].[Product ID] where [sale].[sale Invoice No]= '" + TxtSearchSaleInvoice.Text + "'"
            Dim connection As New SqlConnection(connectionString)
            Dim dataadapter As New SqlDataAdapter(sql, connection)
            Dim ds As New DataSet()
            connection.Open()
        ...
        Try
            ...
            If e.RowIndex >= 0 Then
                ...
                Else
                    ...
                    FrmPOSPasswordConfirmation.Show()
                    FrmPOSPasswordConfirmation.SaleDate = row.Cells("Sale Date").Value
                    FrmPOSPasswordConfirmation.TxtPassword.Focus()
